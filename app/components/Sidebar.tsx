...
export default function Sidebar() {
    // The sidebar uses a fixed position and a dark background (bg-gray-900)
    // with a vibrant accent color (border-red-500) for BCD aesthetic.
    return (
        <div className="w-64 min-h-screen bg-gray-900 border-r border-red-900/50 flex flex-col fixed top-0 left-0 pt-8 z-10">
            ...
            <nav className="flex-1 px-4 py-4 space-y-2">
                {navItems.map((item) => (
                    // Use a dark button style with a hover effect for navigation
                    <Link 
                        key={item.name} 
                        href={item.href} 
